...
import { useRedxParcels } from "@/hooks/useRedxParcels";
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="flex flex-col gap-0.5">
        <span className="font-medium text-sm">{row.original.tracking_id || "N/A"}</span>
      </div>
    ),
  },
  {
...
function CourierPayStatusCell({ invoiceNo }: { invoiceNo: string }) {
  const [payInfo, setPayInfo] = React.useState<{ status: string | null } | null>(null);
  ...
  const { status } = payInfo;
  const formattedStatus = status ? status.replace(/_/g, " ").replace(/\b\w/g, (l) => l.toUpperCase()) : null;
...
function RowActions({ row }: { row: ParcelRow }) {
  const handleCheckStatus = async () => {
    ...
    try {
      const data = await redxService.checkStatus(row.invoice_no);
      const statusText = data?.data?.parcel?.status || "Unknown";
...
export function RedxTable() {
  ...
  const { data, isLoading } = useRedxParcels(pageIndex + 1, debouncedSearch, pageSize);

  const parcelsData = Array.isArray(data) ? data : [];
  const totalCount = parcelsData.length;
  const pageCount = 1;
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="font-normal text-muted-foreground text-sm">RedX Parcels</CardTitle>
                    ...
                    <div className="flex flex-col items-center justify-center gap-3 py-16">
                      ...
                      <div className="space-y-1 text-center">
                        ...
                        <p className="text-muted-foreground text-xs">
                          Orders sent to RedX will be listed here.
                        </p>
                      </div>
                    </div>
                  </TableCell>
