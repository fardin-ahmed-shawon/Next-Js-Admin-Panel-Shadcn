...
import {
  ...
  getCoreRowModel, getFilteredRowModel, getPaginationRowModel, getSortedRowModel, useReactTable, flexRender,
} from "@tanstack/react-table";
import {
  ArrowUpDown, Ban, ChevronDown, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight,
  Download, Edit, Ellipsis, Eye, FileDown, FileText, Filter, MoreHorizontal, Phone, Printer, RefreshCw, Search, ShieldOff, Trash2, Truck, UserX,
} from "lucide-react";
...
const orderStatuses = ["All","Pending","Confirmed","Ready To Ship","In-Courier","Ship Later","Hold","Returned","Pre-Order","Delivered","Cancelled","Missing","Lost","Fake","Trash"] as const;
...
export interface OrderRow {
  ...
  orderType: string; time: string; paid: number; due: number;
  paymentMethod: string; productImages: string[];
  parcelStatus: string; courier: string;
...
const columns: ColumnDef<OrderRow>[] = [
  ...
  { id: "search", accessorFn: (r) => `${r.customer} ${r.id} ${r.phone}`, filterFn: "includesString", enableHiding: true },
  ...
  // Orders column
  { id: "order", header: "Orders",
    cell: ({ row }) => (
      <div className="min-w-[120px]">
        <p className="font-mono text-sm font-semibold">{row.original.id}</p>
        <p className="text-[11px] text-muted-foreground">{row.original.date} · {row.original.time}</p>
      </div>
    ),
  },

  // Customer column
  { id: "customerInfo", header: "Customer",
    cell: ({ row }) => (
      <div className="flex items-center gap-2.5">
        ...
        <div>
          ...
          <Button variant="outline" size="sm" className="mt-1 h-6 gap-1 text-[10px] px-2"
            onClick={() => { navigator.clipboard.writeText(row.original.phone); toast.success("Phone copied!"); }}>
  ...
  // Payment info column
  { id: "paymentInfo", header: "Payment",
    cell: ({ row }) => (
      <div className="min-w-[110px] space-y-0.5">
        <Badge variant="outline" className="text-[10px] h-5 mb-1">{row.original.paymentMethod}</Badge>
        <p className="text-[11px]">Total: <span className="font-semibold tabular-nums">৳{row.original.total.toLocaleString()}</span></p>
        <p className="text-[11px] text-emerald-600">Paid: <span className="font-semibold tabular-nums">৳{row.original.paid.toLocaleString()}</span></p>
        <p className="text-[11px] text-destructive">Due: <span className="font-semibold tabular-nums">৳{row.original.due.toLocaleString()}</span></p>
      </div>
    ),
  },
  ...
  { id: "parcel", header: "Parcel History",
    cell: ({ row }) => (
      <div className="min-w-[120px] text-[11px] space-y-1">
        <div className="flex justify-between"><span className="text-muted-foreground">Total:</span><span className="font-semibold tabular-nums">{row.original.parcelHistory.total}</span></div>
        <div className="flex justify-between"><span className="text-muted-foreground">Delivered:</span><span className="font-semibold tabular-nums">{row.original.parcelHistory.delivered}</span></div>
        <div className="flex justify-between"><span className="text-muted-foreground">Cancelled:</span><span className="font-semibold tabular-nums">{row.original.parcelHistory.cancelled}</span></div>
  ...
  // Order Status column
  { id: "oStatus", header: "Order Status",
    cell: ({ row }) => (
      <Select
...
function exportOrders(data: OrderRow[]) {
  const h = ["Order ID","Customer","Phone","Items","Total","Paid","Due","Order Status","Payment Status","Payment Method","Date"];
  const rows = [h.join(","), ...data.map((r) => [r.id,`"${r.customer}"`,`"${r.phone}"`,r.items,r.total,r.paid,r.due,r.orderStatus,r.paymentStatus,r.paymentMethod,r.date].join(","))];
...
export function OrdersTable({ data }: { data: OrderRow[] }) {
  ...
  const table = useReactTable({
    data, columns,
    state: { rowSelection, columnFilters, sorting, pagination, columnVisibility: { search: false, orderStatus: false, paymentStatus: false, category: false, subCategory: false } },
    getRowId: (r) => r.id,
  ...
  const orderStatusCounts = React.useMemo(() => {
    ...
    data.forEach((o) => { c[o.orderStatus] = (c[o.orderStatus] || 0) + 1; });
    return c;
  }, [data]);
  const paymentStatusCounts = React.useMemo(() => {
    const c: Record<string, number> = {};
    data.forEach((o) => { c[o.paymentStatus] = (c[o.paymentStatus] || 0) + 1; });
    return c;
  }, [data]);
  ...
  function applyOrderFilter(v: string) {
    ...
    table.setPageIndex(0); setRowSelection({});
  }
  ...
  function applyCatFilter(v: string) {
    setActiveCatFilter(v); setActiveSubCatFilter("All");
  ...
  function clearAllFilters() {
    setActiveOrderFilter("All"); setActivePaymentFilter("All"); setActiveCatFilter("All"); setActiveSubCatFilter("All");
    table.resetColumnFilters(); table.setPageIndex(0); setRowSelection({});
    table.getColumn("search")?.setFilterValue(undefined);
  }

  const availableSubs = activeCatFilter !== "All" ? subCategories[activeCatFilter] || [] : [];
  const hasFilters = activeOrderFilter !== "All" || activePaymentFilter !== "All" || activeCatFilter !== "All" || searchQuery;
      ...
      <CardContent className="flex flex-col gap-4 px-0">
        ...
        <div className="flex flex-wrap items-center gap-3 px-4">
          <div className="relative w-full sm:w-auto flex-1 sm:flex-none">
            ...
            <Input className="h-8 w-full sm:w-52 rounded-[min(var(--radius-md),12px)] pl-8" placeholder="Search orders..." value={searchQuery}
              onChange={(e) => { table.getColumn("search")?.setFilterValue(e.target.value || undefined); table.setPageIndex(0); }} />
          </div>
          <Select value={activeCatFilter} onValueChange={applyCatFilter}>
            <SelectTrigger className="h-8 w-[calc(50%-0.375rem)] sm:w-36 text-xs"><SelectValue placeholder="Main Category" /></SelectTrigger>
          ...
          {availableSubs.length > 0 && (
            <Select value={activeSubCatFilter} onValueChange={applySubCatFilter}>
              <SelectTrigger className="h-8 w-[calc(50%-0.375rem)] sm:w-36 text-xs"><SelectValue placeholder="Sub Category" /></SelectTrigger>
              <SelectContent>
          ...
          {hasFilters && <Button variant="ghost" size="sm" className="h-8 text-xs" onClick={clearAllFilters}>Clear filters</Button>}
          <div className="ml-auto flex items-center gap-2">
            <Button size="icon-sm" variant={showFiltersMobile ? "secondary" : "outline"} className="sm:hidden" onClick={() => setShowFiltersMobile(!showFiltersMobile)}>
              <Filter className="size-4" />
            </Button>
            <Button size="icon-sm" variant="outline" className={showFiltersMobile ? "flex" : "hidden sm:flex"} onClick={() => table.getColumn("total")?.toggleSorting(table.getColumn("total")?.getIsSorted() === "asc")} title="Sort by Total"><ArrowUpDown /></Button>
          </div>
        </div>
        ...
        <div className={`flex-col gap-3 ${showFiltersMobile ? 'flex' : 'hidden sm:flex'}`}>
          ...
          <div className="flex flex-col gap-2 px-4">
            <div className="flex items-center gap-2">
              <span className="text-[11px] font-semibold tracking-wider uppercase text-muted-foreground shrink-0">Status:</span>
              <Button size="icon-sm" variant={showStatusFilter ? "ghost" : "outline"} className="h-6 w-6" onClick={() => setShowStatusFilter(!showStatusFilter)} title={showStatusFilter ? "Hide Status Filters" : "Show Status Filters"}>
            ...
            {showStatusFilter && (
              <div className="flex flex-wrap gap-2">
                {orderStatuses.map((s) => (
                  <Button key={s} variant={activeOrderFilter === s ? "default" : "outline"} size="sm" className="h-7 text-xs px-2.5" onClick={() => applyOrderFilter(s)}>
          ...
          <div className="flex flex-col gap-2 px-4">
            <div className="flex items-center gap-2">
              <span className="text-[11px] font-semibold tracking-wider uppercase text-muted-foreground shrink-0">Payment:</span>
              <Button size="icon-sm" variant={showPaymentFilter ? "ghost" : "outline"} className="h-6 w-6" onClick={() => setShowPaymentFilter(!showPaymentFilter)} title={showPaymentFilter ? "Hide Payment Filters" : "Show Payment Filters"}>
            ...
            {showPaymentFilter && (
              <div className="flex flex-wrap gap-2">
                {paymentStatuses.map((s) => (
                  <Button key={s} variant={activePaymentFilter === s ? "default" : "outline"} size="sm" className="h-7 text-xs px-2.5" onClick={() => applyPaymentFilter(s)}>
                    {s} {s === "All" ? `(${data.length})` : paymentStatusCounts[s] ? `(${paymentStatusCounts[s]})` : "(0)"}
                  </Button>
                ))}
              </div>
            )}
          ...
          <Table className="min-w-[1200px]">
            ...
            <TableBody>
              {table.getRowModel().rows.length ? (
                ...
                <TableRow><TableCell colSpan={columns.length} className="h-48">
                  <div className="flex flex-col items-center justify-center gap-2 text-center">
                    <div className="flex size-14 items-center justify-center rounded-full bg-muted"><UserX className="size-6 text-muted-foreground" /></div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-4 pb-2">
          <div className="flex items-center gap-2">
            ...
            <Select value={`${pagination.pageSize}`} onValueChange={(v) => setPagination((p) => ({ ...p, pageSize: Number(v), pageIndex: 0 }))}>
              ...
              <SelectContent>{[5,10,20,50].map((s) => <SelectItem key={s} value={`${s}`}>{s}</SelectItem>)}</SelectContent>
            </Select>
          </div>
          <div className="flex items-center gap-1">
            <span className="text-sm text-muted-foreground mr-2">Page {table.getState().pagination.pageIndex + 1} of {table.getPageCount()}</span>
            <Button size="icon-sm" variant="outline" onClick={() => table.setPageIndex(0)} disabled={!table.getCanPreviousPage()}><ChevronsLeft /></Button>
            <Button size="icon-sm" variant="outline" onClick={() => table.previousPage()} disabled={!table.getCanPreviousPage()}><ChevronLeft /></Button>
            <Button size="icon-sm" variant="outline" onClick={() => table.nextPage()} disabled={!table.getCanNextPage()}><ChevronRight /></Button>
            <Button size="icon-sm" variant="outline" onClick={() => table.setPageIndex(table.getPageCount() - 1)} disabled={!table.getCanNextPage()}><ChevronsRight /></Button>
          </div>
        </div>
      </CardContent>
    </Card>
