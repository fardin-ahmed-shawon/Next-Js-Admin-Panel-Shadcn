...
const colorOptions = ["Red","Blue","Green","Black","White","Yellow","Pink","Purple","Orange","Navy","Maroon","Gray"];
const sizeOptions = ["XS","S","M","L","XL","XXL","3XL","28","30","32","34","36","38","40","42","Free Size"];

const productCatalogue = [
  { id: "PRD-001", name: "Premium Cotton T-Shirt", sku: "SKU-001", price: 850, stock: 124, image: "https://placehold.co/80x80/1a1a2e/e0e0e0?text=TS", category: "Clothing" },
  { id: "PRD-002", name: "Slim Fit Denim Jeans", sku: "SKU-002", price: 1450, stock: 67, image: "https://placehold.co/80x80/1a1a2e/e0e0e0?text=DJ", category: "Clothing" },
  { id: "PRD-003", name: "Wireless Bluetooth Earbuds", sku: "SKU-003", price: 2200, stock: 42, image: "https://placehold.co/80x80/1a1a2e/e0e0e0?text=BE", category: "Electronics" },
  { id: "PRD-004", name: "Leather Crossbody Bag", sku: "SKU-004", price: 3100, stock: 18, image: "https://placehold.co/80x80/1a1a2e/e0e0e0?text=CB", category: "Accessories" },
  { id: "PRD-005", name: "Running Sneakers Pro", sku: "SKU-005", price: 2800, stock: 55, image: "https://placehold.co/80x80/1a1a2e/e0e0e0?text=RS", category: "Footwear" },
  { id: "PRD-006", name: "Organic Face Moisturizer", sku: "SKU-006", price: 650, stock: 200, image: "https://placehold.co/80x80/1a1a2e/e0e0e0?text=FM", category: "Beauty" },
  { id: "PRD-007", name: "Stainless Steel Water Bottle", sku: "SKU-007", price: 480, stock: 310, image: "https://placehold.co/80x80/1a1a2e/e0e0e0?text=WB", category: "Home" },
...
export function CreateOrderForm() {
  ...
  const discountAmount = discountType === "percentage" ? Math.round(subtotal * (Number(discountValue) || 0) / 100) : Number(discountValue) || 0;
  ...
  function resetForm() {
    ...
    setPaymentMethod("cod"); setPaymentStatus("unpaid"); setTransactionId("");
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="flex flex-col gap-6 lg:col-span-2">
          ...
          <Card>
            ...
            <CardContent className="flex flex-col gap-4">
              <div ref={searchRef} className="relative">
                ...
                <Input className="pl-9" placeholder="Search by product name, SKU, or ID..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onFocus={() => setSearchFocused(true)} />
                {searchFocused && filteredProducts.length > 0 && (
                  <div className="absolute left-0 right-0 top-full z-50 mt-1 max-h-72 overflow-y-auto rounded-lg border bg-popover shadow-lg">
                    {filteredProducts.map((p) => {
                      ...
                      return (
                        <button key={p.id} type="button" className="flex w-full items-center gap-3 px-3 py-2.5 text-left transition-colors hover:bg-muted/50" onClick={() => addToCart(p)}>
                          <div className="size-10 shrink-0 overflow-hidden rounded-md border bg-muted"><img src={p.image} alt={p.name} className="size-full object-cover" /></div>
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            ...
                            {inCart && <Badge variant="secondary" className="text-[10px]">×{inCart.quantity}</Badge>}
                          </div>
                        </button>
                      );
                    })}
              ...
              {cart.length === 0 ? (
                ...
                <div className="flex flex-col gap-3">
                  {cart.map((item) => (
                    <div key={item.product.id} className="rounded-lg border p-3 transition-colors hover:bg-muted/30">
                      <div className="flex items-center gap-3">
                        <div className="size-12 shrink-0 overflow-hidden rounded-md border bg-muted"><img src={item.product.image} alt={item.product.name} className="size-full object-cover" /></div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium truncate">{item.product.name}</p>
                          <p className="text-xs text-muted-foreground">{item.product.sku} · ৳{item.product.price.toLocaleString()} each</p>
                        </div>
                        ...
                        <span className="w-20 text-right text-sm font-semibold tabular-nums">৳{(item.product.price * item.quantity).toLocaleString()}</span>
                      ...
                      <div className="mt-2 flex items-center gap-3 pl-15">
                        <Select value={item.color} onValueChange={(v) => updateCartItem(item.product.id, "color", v)}>
                          <SelectTrigger className="h-7 w-28 text-xs"><SelectValue placeholder="Color" /></SelectTrigger>
                        ...
                        <Select value={item.size} onValueChange={(v) => updateCartItem(item.product.id, "size", v)}>
                          <SelectTrigger className="h-7 w-28 text-xs"><SelectValue placeholder="Size" /></SelectTrigger>
                  ...
                  <div className="flex items-center justify-between rounded-md bg-muted/50 px-3 py-2">
                    <span className="text-sm text-muted-foreground">{cart.reduce((s, i) => s + i.quantity, 0)} item(s)</span>
            ...
            <CardContent className="flex flex-col gap-5">
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2 sm:col-span-2">
                  ...
                  <Input id="customer-name" placeholder="e.g. Arham Khan" value={customerName} onChange={(e) => setCustomerName(e.target.value)} />
                </div>
                ...
                <div className="space-y-2">
                  <Label htmlFor="customer-phone">Phone <span className="text-destructive">*</span></Label>
                  <Input id="customer-phone" type="tel" placeholder="+880 1XXX-XXXXXX" value={customerPhone} onChange={(e) => setCustomerPhone(e.target.value)} />
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="flex flex-col gap-5">
              <div className="space-y-2">
                <Label htmlFor="shipping-address">Street Address <span className="text-destructive">*</span></Label>
                <Textarea id="shipping-address" placeholder="House #, Road #, Block, Area..." className="min-h-[80px] resize-y" value={shippingAddress} onChange={(e) => setShippingAddress(e.target.value)} />
              </div>
              <div className="grid gap-4 sm:grid-cols-3">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={thana} onValueChange={setThana} disabled={!district}>
                    <SelectTrigger className="w-full"><SelectValue placeholder="Select thana" /></SelectTrigger>
                    <SelectContent>{availableThanas.map((t) => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
              </div>
              <Separator />
        ...
        <div className="flex flex-col gap-6">
          ...
          <Card>
            ...
            <CardContent className="flex flex-col gap-5">
              ...
              <div className="space-y-2">
                <Label className="text-primary font-medium">Payment Status</Label>
              ...
              {paymentMethod !== "cod" && (
                <div className="space-y-2">
                  <Label htmlFor="txn-id" className="text-primary font-medium">Transaction ID</Label>
                  <Input id="txn-id" placeholder="e.g. TXN123456789" value={transactionId} onChange={(e) => setTransactionId(e.target.value)} />
                </div>
              )}
            </CardContent>
          </Card>
            ...
            <CardContent className="flex flex-col gap-4">
              <div className="grid gap-3 grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input type="number" placeholder={discountType === "percentage" ? "e.g. 10" : "e.g. 200"} value={discountValue} onChange={(e) => setDiscountValue(e.target.value)} />
                </div>
              </div>
              {discountAmount > 0 && <p className="text-xs text-emerald-600 font-medium">Discount applied: −৳{discountAmount.toLocaleString()}</p>}
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="flex flex-col gap-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="order-note" className="text-primary font-medium">Order Note</Label>
                <Textarea id="order-note" placeholder="Internal notes about this order..." className="min-h-[80px] resize-y" value={orderNote} onChange={(e) => setOrderNote(e.target.value)} />
              </div>
            </CardContent>
          </Card>
          ...
          {/* Order Summary */}
          <Card>
            <CardHeader><CardTitle className="text-base text-primary">Order Summary</CardTitle></CardHeader>
